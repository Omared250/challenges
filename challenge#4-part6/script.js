...
// 1.
dogs.forEach((dog) => {
    const food = dog.recommendedFood = Math.trunc(dog.weight ** 0.75 * 28);
    return food;
})
console.log(dogs);
...
// 4.
const strTooMuch = `${ownersEatTooMuch.join(' and ')}'s dogs eat too much!`
console.log(strTooMuch);

const strTooLittle = `${ownersEatTooLittle.join(' and ')}'s dogs eat too little!`
...
// 6.
const rightPortionFood = dog => 
    dog.curFood > (dog.recommendedFood * 0.90) && 
    dog.curFood < (dog.recommendedFood * 1.10);

console.log(dogs.some(rightPortionFood));

// 7.
const dogWithGoodAmountOfFood = dogs.filter(rightPortionFood);
